%하이브리드 힘/위치 제어 연습당시 파일임
%데모 파일과 다른 점은 Xd/Xe의 위치 및 구현이 다름. (힘->위치 스위치시 틱거림은 없지만, 실제값인 Xe를 바꿔서 구현한 잘못된 구현임)
%%
clear *;
close all;
clc
...
%         ePgain = [100 100 250 20 20 10];  
        ...
        %힘제어
        if ControlSwitch == 1            
            Fc_force = [0 0 0.3*(Fe(3) - desired_force) 0 0 0];
            Tc_force = (J' * Fc_force')';
        else
