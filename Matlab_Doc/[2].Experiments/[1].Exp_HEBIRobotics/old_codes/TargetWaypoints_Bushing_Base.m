function [xyzTargets, rotMatTarget, control_time, gripperforce] = TargetWaypoints(demo_case)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 지그로부터 부싱 pick %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                         %1       2       3       4   
    pick.xyzTargets =   [ 0.290   0.290   0.290   0.290   0.270;    % x [m]
                         -0.270  -0.270  -0.270  -0.270  -0.270;    % y [m]
                         -0.080  -0.080  -0.207  -0.207   0.000];  % z [m]  
    pick.gripperforce = [-5       -5      2      -5      -5];  %동작 시작시에 해당 힘 완료함    
    pick.control_time = [ 3       1       3.0     0.8     1]; 
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    insert_end.xyzTargets =    [ 0.470   0.470   0.470   0.470  0.470;    % x [m]
                                 0.040   0.095   0.089   0.089  0.089;    % y [m]
                                 0.100   0.100  -0.025  -0.025  0.070];  % z [m]   
    insert_end.gripperforce =  [-5      -5      -5       3      3    ];  %동작 시작시에 해당 힘 완료함 
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    pushing.xyzTargets =   [ 0.510   0.510   0.510;    % x [m]
                            ...
                            -0.048  -0.048  -0.048];  % z [m]
    pushing.gripperforce = [-5      -1.4    -5 ]; %실제 부싱은 -1.3           
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 실험 세팅으로 이동 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                          %1       2          
    Moveto.xyzTargets =   [-0.010  -0.010;    % x [m]
                           -0.250  -0.250;    % y [m]
                           -0.100  -0.100 ];  % z [m]
    Moveto.gripperforce = [-5      -5     ];           
    Moveto.control_time = [ 5       2   ];
    Moveto.rotMatTarget = [ R_x(pi) R_x(pi)];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    elseif demo_case == 14
            %%%%%%%% 특수메탈 실험세팅후 pick&pushing&insert %%%%%%%%%%
        xyzTargets =       [pick.xyzTargets     Moveto.xyzTargets  ];
        gripperforce =     [pick.gripperforce   Moveto.gripperforce];           
        control_time =     [pick.control_time   Moveto.control_time];        
        rotMatTarget_Set = [pick.rotMatTarget   Moveto.rotMatTarget]; 
        for i = 1 : size(xyzTargets,2)          
            rotMatTarget{i} = rotMatTarget_Set(:,3*i-2:3*i);          
        end  
