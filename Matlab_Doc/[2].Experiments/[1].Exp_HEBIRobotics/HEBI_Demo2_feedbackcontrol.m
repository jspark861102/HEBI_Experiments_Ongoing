% 헤비로봇 세팅 초기 단계때 파일로써, 정해진 위치 (사각형)으로 차례대로 위치 제어 하는 파일
% 헤비 로봇 예제는 위치,속도,토크 레퍼런스를 활용한데 반해, 나는 토크 레퍼런스만 활용하여 구현함
% 핸드폰 연결, 그리퍼 제어 등이 구현되어 있음
% 단축 (joint space) 수준의 충돌검지도 구현되어 있음
